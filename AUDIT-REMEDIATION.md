...
| ARC-016 / ARC-012 follow-up | ✅ | fix-architecture (opus) | ARC-016, ARC-012 (Rust files) | 2 | 0 | 0 |
| 4 — Verification | ✅ | orchestrator | — | — | — | — |

**Overall**: of 55 unique issues — **50 resolved**, **1 partial**, **3 deferred (require manual/cross-repo work)**, **1 accepted as-is (optional per audit)**. The full project gate (`make checkall` = Rust fmt + typecheck + clippy `-D warnings` + test + security-audit, plus the new `web-checkall`) is green with zero test failures (318 Rust + 11 web).

### Remediation commits (on `fix/audit-remediation`)

```
21e333f refactor(architecture): ARC-012 — split loader/mesh.rs into per-area submodules
c053c3e refactor(architecture): ARC-012 — split point_feature.rs into per-area submodules
0717023 refactor(architecture): ARC-012 — split road/mod.rs into per-area submodules
e0ed9fe fix(architecture): ARC-016 — consolidate three append_box impls into one shared primitive
...
~110 files changed across 11 atomic commits. Each commit is a clean rollback point.
...
### Architecture (16 resolved, 0 partial, 1 deferred, 1 accepted)
...
- **[ARC-016]** ✅ Test scene gated behind `cfg(any(test, feature = "dev_scene"))` (release builds exclude it). The three `append_box` impls (`buffers.rs`, `road/mod.rs`, `point_feature.rs`) are **consolidated into one shared `crate::mesh::append_box` + `BoxSpec`** (no semantic divergence — all produced 24 verts / 36 indices with identical normals; the road slab-inflation edge case preserved). Thin domain wrappers bake in each caller's default `feature_type`. 2 sanity tests added.
- **[ARC-012]** ✅ Decomposed the Rust god-files into per-area submodules using the **re-export-hub pattern** (parent `mod.rs` re-exports everything → external call sites unchanged, verified by `cargo check`):
  - `src/world/road/mod.rs`: **1688 → 101 lines** + `profile`, `render_path`, `ribbon`, `centerline`, `cap`, `geometry`, `tests` (23 tests preserved).
  - `src/world/point_feature.rs` → directory: **1385 → 111 lines** (`mod.rs`) + `style`, `tree`, `landmark`, `nature`, `poi`, `transit`, `geometry`, `tests` (17 tests preserved).
  - `src/world/loader/mesh.rs` → directory: **960 → 527 lines** (`mod.rs`) + `terrain_cuts`, `water`, `roads`; bonus DRY fix de-duplicating the road-cap counting/emission logic.
  - `web/src/app/page.tsx` deliberately **not** touched here — that's QA-001 (separate, delicate React refactor).
...
### [ARC-012] Decompose Rust god-files — ✅ RESOLVED (see Architecture section)
- `road/mod.rs`, `point_feature.rs`, `loader/mesh.rs` all split via re-export hubs; external call sites unchanged.

### [QA-001] / [QA-007] Decompose `page.tsx` + peel test modules — DEFERRED (backlog)
- **Why**: `web/src/app/page.tsx` (`Home` component, ~969 LOC) is a delicate React refactor (28 `useState`, interwoven effects/handlers). Doing it blind with par-mem down risked breaking a working UI; it's a pure refactor with no behavior/bug impact, so deferring leaves the app working. QA-007 (peel test modules into sibling `tests.rs`) overlaps and was partly absorbed — the road/point_feature splits already moved their tests into `tests.rs` submodules.
- **Recommended approach**: a dedicated, carefully-tested pass — ideally with par-mem back online for reference sweeps, in a worktree, behind Playwright screenshot tests for the web side.

### [ARC-016] Consolidate `append_box` — ✅ RESOLVED (see Architecture section)

### [QA-010] (partial) Drop clones in `generate_road_with_elevations_and_feature_type`
- **Why partial**: requires changing the function to owned-`Vec` signatures, rippling into `src/world/loader/mesh.rs`. Now that `loader/mesh.rs` is split (ARC-012), the ripple is more contained — fold into a future pass.
...
| Rust tests | ✅ 0 failures (318 in the binary crate: 286 lib + 17 server + 11 + 4 shader; plus 11 web) |
...
~110 files across 11 commits: 10 security-server files, ~40 documentation files (README/CONTRIBUTING/AGENTS/ARCHITECTURE/troubleshooting/ideas + 26 superpowers specs/plans + LICENSE + docstrings/JSDoc), the shader build pipeline (`build.rs`, `features.wgsl`, `scene_uniforms.wgsl`, city/sky.wgsl, pipelines/sky_pipeline.rs, shader_source_test.rs), the workspace manifests + `Cargo.lock`, the web tooling (`web/package.json`, `eslint.config.mjs`, `Makefile`, `.pre-commit-config.yaml`), the **god-file decomposition** (road/ + point_feature/ + loader/mesh/ submodule splits), and the ARC-003 deletion of the vendored `crates/par-osm-rust/`. See `git diff --stat 4b80492..HEAD` for the full list.

---

## Next Steps

1. **Review the 4 remaining items** (ARC-004 deferred, QA-001/QA-007 deferred, QA-010 partial). ARC-004 is the lead — it needs an upstream PR to add the tagged-node parse API the renderer wants, then this repo can delete `src/osm/parse.rs`/`src/geo/elevation.rs`/`src/geo/srtm.rs` and consume the published crate from the renderer too (currently only the server consumes it).
2. **`page.tsx` decomposition (QA-001)** is the remaining large refactor — recommend a dedicated pass with par-mem back online + Playwright screenshot tests. QA-007 (peel test modules) is mostly absorbed (road/point_feature tests already moved to `tests.rs` submodules in ARC-012).
