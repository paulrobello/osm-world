...
//!
//! Supports SRTM1 (3601x3601, ~30 m resolution) and SRTM3 (1201x1201, ~90 m
//! resolution) tiles.  Each tile is a 1x1 degree cell named after its SW corner:
...
/// A single SRTM HGT tile covering a 1x1 degree cell.
///
...
impl HgtTile {
    ...
    fn load(path: &Path) -> io::Result<Self> {
        ...
        let size = match n_samples {
            1_442_401 => 1201,  // SRTM3 (1201 x 1201)
            12_967_201 => 3601, // SRTM1 (3601 x 3601)
            _ => {
...
#[cfg(test)]
impl HgtTile {
    /// Create a synthetic tile of arbitrary `size x size` from a flat slice
    /// of i16 elevation values (big-endian, row-major).
    ///
    /// Bypasses the SRTM1/SRTM3 size validation so unit tests can use small
    /// (e.g. 2x2) grids without allocating multi-megabyte fixtures.
...
mod tests {
    ...
    fn tile_elevation_at_corners() {
        // 2x2 tile: NW=100, NE=200, SW=300, SE=400
    ...
    fn tile_elevation_centre_bilinear() {
        // Centre of a 2x2 tile -> average of all four = (100+200+300+400)/4 = 250
        let tile = HgtTile::synthetic(0, 0, 2, &[100, 200, 300, 400]).unwrap();
