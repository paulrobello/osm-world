...
fn sky_shader_sun_uses_layered_depth_terms() {
    ...
    let shader = if let Some(pos) = shader_raw.find("// --- Sky helpers (loaded from sky_helpers.wgsl at compile time) ---") {
        ...
        combined.push_str(&shader_raw[pos + "// --- Sky helpers (loaded from sky_helpers.wgsl at compile time) ---".len()..]);
        // Remove the fog helpers placeholder (already in sky_helpers.wgsl)
        if let Some(fog_pos) = combined.find("// --- Fog helpers (loaded from sky_helpers.wgsl at compile time) ---") {
            combined.replace_range(fog_pos..fog_pos + "// --- Fog helpers (loaded from sky_helpers.wgsl at compile time) ---".len(), "");
        }
        combined
    } else {
